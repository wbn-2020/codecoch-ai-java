...
阶段五 README 口径：后端重点展示 AI 求职训练闭环、AI 建议证据链、个人知识库、长期记忆、异步任务、Prompt/日志治理和演示数据隔离能力；不把规划能力写成已完成能力，也不扩展到真实企业入驻、支付、自动投递或 OfferLab 社区。
...
第三次拓展阶段五不是新增大型后端功能，而是把已有能力整理成面试可讲、README 可读、发布后可人工验收的作品集版本。

| 能力 | 后端口径 |
|---|---|
| 目标岗位、JD 解析、简历/JD 匹配、能力画像 | V3 已有核心链路，可作为已完成主闭环说明 |
| 题库训练、模拟面试、面试报告、学习计划 | V1/V2/V3 已有训练链路，可作为核心训练能力说明 |
| Prompt 模板、Prompt 版本、AI 调用日志、traceId | 已有 AI 工程化基础，可作为 AI Ops 能力说明 |
| JobCoachAgent、今日任务、任务完成/跳过、运行详情 | V4-A MVP 主路径，可作为 Agent 任务闭环说明 |
| 求职实验台 | 当前按 MVP/增强方向说明，完整策略实验和样本可信度仍需发布后验收确认 |
| AI 建议证据链 | 第三次拓展 P0 可信化能力，README 中按重点增强/MVP 样例说明 |
| 个人知识库、长期记忆 | 按私域知识资产和长期记忆治理方向说明，不写成跨用户知识共享 |
...
如果只验证 V1/V2 基础接口，通常只需要 MySQL、Redis、Nacos；如果验证异步任务、搜索、V3 工程化能力，再按需启用 RocketMQ、Elasticsearch 或 Qdrant。阶段五静态收口期间不要在本地启动这些服务。
...
阶段五静态收口期间不要在本地执行编译、测试、服务启动、Docker、`java -jar` 或任何会监听端口、构建项目、运行测试、长期占用 CPU 的命令。阶段五本地只允许阅读文件、搜索文本、检查 Markdown 结构和编辑 README；发布后再由主 Agent 或用户统一执行真实服务、接口、AI 调用和构建验收。
