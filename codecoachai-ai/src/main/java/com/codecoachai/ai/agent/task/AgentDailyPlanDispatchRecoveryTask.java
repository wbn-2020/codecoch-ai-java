...
public class AgentDailyPlanDispatchRecoveryTask {
    ...
    private void recoverCandidate(Map<String, Object> row) {
        ...
        if (runId == null || userId == null || !StringUtils.hasText(executionToken)) {
            log.warn("Skip agent daily plan dispatch recovery row because required fields are missing: {}",
                    safeRowSummary(row));
            return;
        }
    ...
    private Map<String, Object> safeRowSummary(Map<String, Object> row) {
        ...
        summary.put("targetJobId", toLong(row.get("targetJobId")));
        summary.put("planDate", toLocalDate(row.get("planDate")));
        ...
        summary.put("rowHash", shortHash(row.keySet() + "|"
                ...
                + toLong(row.get("targetJobId")) + "|"
                + toLocalDate(row.get("planDate")) + "|"
