...
public class ApplicationWorkspaceServiceImpl implements ApplicationWorkspaceService {
    ...
    public WorkspaceView get(Long applicationId) {
        ...
        view.getCoverage().put("application", new Coverage("codecoachai-core", true, 1, false));
        ...
        view.getCoverage().put("nextSteps",
                new Coverage("codecoachai-core", true, view.getNextSteps().size(), false));
        view.getCoverage().put("mockInterviewEvidence",
                new Coverage("codecoachai-core", false, 0, false));
        view.getWarnings().add("模拟面试证据暂不可用，本次工作区仅返回 Core 已确认的事实。");
        return view;
    }
    ...
    private void loadTimeline(WorkspaceView view, Long userId, Long applicationId) {
        try {
            ...
            view.getCoverage().put("timeline",
                    new Coverage("codecoachai-core", true, view.getTimeline().size(), truncated));
        } catch (RuntimeException ex) {
            view.getCoverage().put("timeline", new Coverage("codecoachai-core", false, 0, false));
    ...
    private void loadCalendar(WorkspaceView view, Long userId, Long applicationId) {
        try {
            ...
            view.getCoverage().put("calendar",
                    new Coverage("codecoachai-core", true, view.getCalendar().size(), truncated));
        } catch (RuntimeException ex) {
            view.getCoverage().put("calendar", new Coverage("codecoachai-core", false, 0, false));
    ...
    private void loadMaterials(WorkspaceView view, Long userId, Long applicationId) {
        try {
            ...
            view.getCoverage().put("materials",
                    new Coverage("codecoachai-core", true, view.getMaterials().size(), truncated));
        } catch (RuntimeException ex) {
            view.getCoverage().put("materials", new Coverage("codecoachai-core", false, 0, false));
