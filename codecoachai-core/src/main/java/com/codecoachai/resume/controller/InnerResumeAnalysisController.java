...
/**
 * 简历解析任务相关 inner 接口，供 Core 内置异步消费者调用。
 *
 * <ul>
...
public class InnerResumeAnalysisController {
    ...
    private static final String CORE_SERVICE_NAME = "codecoachai-core";
    ...
    public Result<RawVO> getAnalysisRaw(@PathVariable("id") Long id,
                                        ...
                                        String serviceName) {
        requireCoreService(serviceName);
    ...
    private void requireCoreService(String serviceName) {
        if (!CORE_SERVICE_NAME.equals(serviceName)) {
            throw new BusinessException(ErrorCode.FORBIDDEN, "Resume analysis inner API only accepts core service");
        }
    }
