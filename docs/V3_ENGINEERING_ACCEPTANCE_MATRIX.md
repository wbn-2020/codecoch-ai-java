...
> Agent C 负责范围：Task/File 业务域、`codecoachai-search`、`codecoachai-common/common-redis`、
> `codecoachai-common/common-mq`。本矩阵仅按当前代码与配置静态核对，未覆盖 P0 文件范围。
> 第二阶段迁移后，Task/File 的源码、测试和控制器路径统一位于 `codecoachai-core`；下方
> `com.codecoachai.task`、`com.codecoachai.file` 包名保持不变。Task/File 的 Maven 验证命令
> 使用 `-pl codecoachai-core`；Search 和 Common 仍使用各自的 Maven 模块路径，不能套用 Core
> 命令。
